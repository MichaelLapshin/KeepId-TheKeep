...
/**
 * Test_ValidJsons - Test that a valid data field Jsons do not cause
 *                   any error to occur when being asserted for correctness.
 */
TEST(TestDataField_Assertions, Test_ValidJsons){
    ...
    EXPECT_NO_THROW(Assertions::assertAreConfigDataFields(data_fields));
    // EXPECT_NO_THROW(Assertions::assertValidDataUpdate(data_update_input)); // TODO, Needs to be rewritten
    // EXPECT_NO_THROW(Assertions::assertValidDataRequest(data_request_input)); // TODO, Needs to be rewritten
...
/**
 * TestMethodThrows_assertAreConfigDataFields - Test that the method assertAreConfigDataFields() correctly 
 *              throws when invalid data_fields (compared to Config) are passed within the list.
 */
TEST(TestDataField_Assertions, TestMethodThrows_assertAreConfigDataFields){
    // Sets up the test
    copyFile(SAMLE_DATA_FIELDS + "config_valid.json", DATA_FIELD_CONFIG_FILE_NAME);
    Config::initialize();
    ...
    // Tests that assertAreConfigDataFields() throws as expected
    EXPECT_THROW(Assertions::assertAreConfigDataFields(dt_empty), runtime_error);
    EXPECT_THROW(Assertions::assertAreConfigDataFields(dt_empty_str), runtime_error);
    EXPECT_THROW(Assertions::assertAreConfigDataFields(dt_one), runtime_error);
    EXPECT_THROW(Assertions::assertAreConfigDataFields(dt_invalid_some), runtime_error);
    EXPECT_THROW(Assertions::assertAreConfigDataFields(dt_invalid_all), runtime_error);
...
TEST(TestDataField_Assertions, TestMethodThrows_assertValidDataUpdate){
    ...
    Config::initialize();
     ...
     EXPECT_TRUE(false); // TODO, this test needs to be rewritten
...
TEST(TestDataField_Assertions, TestMethodThrows_assertValidDataRequest){
    ...
    // EXPECT_THROW(Assertions::assertValidDataRequest(extractJsonValue(INVALID_REQUEST + "request_invalid_private_public_keys_entry_invalid.json")), runtime_error);
